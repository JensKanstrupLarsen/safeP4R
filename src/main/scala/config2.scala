  ...
  override def toProto(te : safeFP4R.TableEntry[TableMatchFields, TableAction, ActionParams, _, _]) : TableEntry =
    ...
    val matchFields =
      (te.table, te.matches) match
        case ("*", _) => Seq.empty
        case (_, _ : "*") => Seq.empty
        case ("Process.firewall", (t0) : (Option[("hdr.ipv4.dstAddr", LPM)])) => t0.map((_, t) => safeFP4R.matchFieldToProto(1, t)).toSeq
        case ("Process.ipv4_table", (t0) : (Option[("hdr.ipv4.dstAddr", LPM)])) => t0.map((_, t) => safeFP4R.matchFieldToProto(1, t)).toSeq
