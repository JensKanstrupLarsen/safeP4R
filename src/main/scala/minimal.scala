...
Using a match type to type a parameter in a polymorphic function (e.g. f[A](x : M[A]))
causes a compiler crash if the function is called with a type argument that does not
reduce the match type and the term argument is not a basic type (e.g. f[String](Some(1))
or f[String](List(1))). Error is reported as normal even if the match type does not
reduce but argument is a basic type (e.g. f[String](1)).
