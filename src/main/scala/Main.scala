...
def genImports() : RIO[Unit, String] = ZIO.succeed(
  "import safeFP4R.{Exact, LPM, Optional, Range, Ternary, SafeFP4RuntimeObserver}\n" +
  "import com.google.protobuf.ByteString\n" +
...
def genMatchFieldToProtoCase(table : String, mfs : Seq[MatchField]) : RIO[Unit, String] = for {
  ...
  caseVarType <- genMatchFieldArgs(mfs)
  ...
  caseResult <- mapM(fieldsIndexed, (mf,idx) => {
    mf.`match`.matchType match
      ...
      case Some(value) =>
        value match
          case EXACT => ZIO.succeed("Seq(safeFP4R.matchFieldToProto(" + mf.id + ", t" + idx + "))")
          case _ => ZIO.succeed("t" + idx + ".map((_, t) => safeFP4R.matchFieldToProto(" + mf.id + ", t)).toSeq")
  })
} yield {
  if caseVars.size > 0 && caseResult.size > 0 then
    "        case (\"" + table + "\", (" + caseVars.reduce((v1, v2) => v1 + ", " + v2) + ") : " + caseVarType + ") => " + caseResult.reduce((v1, v2) => v1 + " ++ " + v2)
  else
...
def genToProto(p4info : P4Info) : RIO[Unit, String] = for {
  ...
  "  override def toProto(te : safeFP4R.TableEntry[TableMatchFields, TableAction, ActionParams, _, _]) : TableEntry =\n" +
  table + "\n" +
  matchFields + "\n" +
  action + "\n" +
  params + "\n" +
...
def genFromProto(p4info : P4Info) : RIO[Unit, String] = for {
  ...
  "  override def fromProto[TM[_], TA[_], TP[_], XN <: String, XA <: TA[XN]](te : TableEntry): safeFP4R.TableEntry[TM, TA, TP, XN, XA] =\n" +
...
def genChannel(p4info : P4Info) : RIO[Unit, String] = for {
  ...
  "class Chan (deviceId : Int, socket : P4RuntimeStub, channel : io.grpc.ManagedChannel) extends safeFP4R.Chan[TableMatchFields, TableAction, ActionParams](deviceId, socket, channel):\n" +
  toProto + "\n" +
  fromProto
}
// === `connect` API ===
def genConnect(p4info : P4Info) : RIO[Unit, String] = ZIO.succeed(
  ...
  "  val response_obs = new SafeFP4RuntimeObserver[StreamMessageResponse](StreamMessageResponse())\n" +
