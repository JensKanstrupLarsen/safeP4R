  ...
  override def toProto(te : safeP4R.TableEntry[TableMatchFields, TableAction, ActionParams, _, _]) : TableEntry =
    ...
    val matchFields =
      (te.table, te.matches) match
        case ("*", _) => Seq.empty
        case (_, _ : "*") => Seq.empty
        case ("Process.nat_ingress", (t0, (_, t1))) => t0.asInstanceOf[Option[("hdr.ipv4.dstAddr", LPM)]].map((_, t) => safeP4R.matchFieldToProto(1, t)).toSeq ++ Seq(safeP4R.matchFieldToProto(2, t1.asInstanceOf[Exact]))
        case ("Process.nat_egress", (t0, (_, t1))) => t0.asInstanceOf[Option[("hdr.ipv4.srcAddr", LPM)]].map((_, t) => safeP4R.matchFieldToProto(1, t)).toSeq ++ Seq(safeP4R.matchFieldToProto(2,  t1.asInstanceOf[Exact]))
